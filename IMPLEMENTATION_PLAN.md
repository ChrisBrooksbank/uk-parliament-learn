...
- [ ] Install YAML loader dependency (vite plugin + js-yaml) and add TypeScript types for Topic, Category, Glossary matching `_schema.yaml` (spec: content-loading.md)
- [ ] Implement content loader: load all 36 topic YAML files and `_categories.yaml` into memory with in-memory cache (spec: content-loading.md)
...
- [ ] Create `index.html` app shell with header, main content area, and nav; wire up Vite entry point (spec: core-ui-navigation.md)
...
- [ ] Implement Glossary page: alphabetically sorted terms with definitions (spec: core-ui-navigation.md)
